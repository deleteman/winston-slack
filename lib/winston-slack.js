...
var nodeSlack = require('slack-notify'),
    util = require('util'),
...
var Slack = exports.Slack  = function (options) {
    ...
    if(!options.apiToken || !options.domain){
        ...
        var slackWebHookURL = 'https://' + this.domain + '.slack.com/services/hooks/incoming-webhook?token=' + this.apiToken;

        //- Configure node-slack module
        this.slack = nodeSlack(slackWebHookURL);
    }
};
...
Slack.prototype.log = function (level, msg, meta, callback) {
    ...
    this.slack.send(message, callback);
};
